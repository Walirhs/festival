...
;(function($) { // TODO REGARDER D'OU SORT CE PUTAIN DE 25 (et retirer le forcing du $size à 10)
	'use strict';
		...
		tablesorterPager: new function() {
			...
			this.defaults = {
                ...
                ajaxError: function( config, xhr, settings, exception ){
                    if (typeof exception !== 'undefined') {
                        toastr.error("Impossible d'exécuter la recherche.");
                        return exception;
                    }
                    return false;
                },
                ...
                ajaxProcessing: function(data) {
                    if (data && data.hasOwnProperty('rows')) {
                        ...
                        var headerXref = headers.join(',').replace(/\s+/g,'').split(',');
                        var rows = [];
                        var len = d.length;
				...
				cssFirst: '.pager-first', // go to first page arrow
				cssPrev: '.pager-prev', // previous page arrow
				cssNext: '.pager-next', // next page arrow
				cssLast: '.pager-last', // go to last page arrow
				cssGoto: '.pager-goto', // go to page selector - select dropdown that sets the current page
				cssPageDisplay: '.pagedisplay', // location of where the "output" is displayed
				cssPageSize: '.pagesize', // page size selector - select dropdown that sets the "size" option
				cssErrorRow: 'table-danger', // error information row
			...
			updatePageDisplay = function(table, p, completed) {
				...
				if ( getTotalPages( table, p ) >= 0 ) {
					...
					// if ( $el.length ) { // MODIF par Sosthèn pour gérer la pagination depuis les numéros
					// 	t = '';
			...
			init = function(table, settings) {
				...
				if (p.savePages && ts.storage) {
					...
					// p.page = isNaN(t.page) ? p.page : t.page; // FIXED SGD
